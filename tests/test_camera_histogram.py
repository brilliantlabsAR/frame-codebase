...
async def capture_and_download(b: Bluetooth):
    ...
    # print("Exposing")
    # await b.send_lua("frame.camera.set_gain(245)")
    # await b.send_lua("frame.camera.set_shutter(95)")
...
async def main():
    ...
    def update_graph(response: str):
        ...
        if color == 'r':
            red_bars.remove()
            red_bars = ax[0].bar(x, data, color='red')
        elif color == 'g':
            green_bars.remove()
            green_bars = ax[1].bar(x, data, color='green')
        elif color == 'b':
            blue_bars.remove()
            blue_bars = ax[2].bar(x, data, color='blue')
