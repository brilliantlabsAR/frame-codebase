...
$(BUILD_FOLDER_PATH)/fpga_rtl_evn.bit: $(FPGA_RTL_SOURCE_FILES)
	...
	cd $(MAKEFILE_PATH) && iverilog -Wall -g2012 -o /dev/null -i top.sv
	yosys -p "synth_nexus -json $(BUILD_FOLDER_PATH)/fpga_rtl.json" $(MAKEFILE_PATH)/top.sv
	nextpnr-nexus --device LIFCL-40-8BG400 \
			      --pdc $(MAKEFILE_PATH)/evn_pinout.pdc \
				  --json $(BUILD_FOLDER_PATH)/fpga_rtl.json \
				  --fasm $(BUILD_FOLDER_PATH)/fpga_rtl.fasm
	prjoxide pack $(BUILD_FOLDER_PATH)/fpga_rtl.fasm $@
